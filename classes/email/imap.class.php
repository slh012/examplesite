...
class imap extends email
{
    ...
    public function fetch_header($email_number='0',$options='0'){
        //debug::output("Fetching Header...");
        $header =  imap_fetchheader($this->imap,$email_number,$options);
        if(empty($header))
        {
                debug::output("No headers...");
            //throw new emailException(imap_last_error($this->imap),imap_errors($this->imap));
        }else{
            return  $header;
        }
